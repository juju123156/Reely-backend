...
def _ensure_dotenv() -> None:
    """Load .env (base) then .env.{APP_ENV} (override) from the first found candidate dir.

    APP_ENV controls which environment-specific file is loaded on top of the base .env.
    Defaults to "paper" when APP_ENV is not set.

    Examples:
        APP_ENV=paper  →  .env  +  .env.paper   (모의투자)
        APP_ENV=live   →  .env  +  .env.live    (실거래)
    """
    ...
    if base_path is not None:
        _load_env_file(base_path, override=False)

    app_env = os.environ.get("APP_ENV", "paper").strip().lower()
    if base_path is not None:
        env_specific = base_path.parent / f".env.{app_env}"
        if env_specific.exists():
            _load_env_file(env_specific, override=True)
