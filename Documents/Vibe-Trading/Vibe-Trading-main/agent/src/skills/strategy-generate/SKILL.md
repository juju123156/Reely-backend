...
### Orchestrator Ranking Artifact

If the strategy is cross-sectional and the orchestrator needs to choose the final trade targets, write a ranking artifact from `signal_engine.py`:

```python
from src.trading.signal_ranking_artifact import export_latest_signal_scores
```

After you build `signal_map`, call:
...
This writes `artifacts/symbol_scores.json` automatically by reading `VIBE_TRADING_RUN_DIR` from the runtime environment. The artifact can then be consumed by `apply_signal_ranking_selection(...)`.

**Default rule for KR orchestrator requests:**
- If the request is for Korea-market orchestration and the strategy considers more than one symbol, you should treat `export_latest_signal_scores(...)` as the default, not an optional add-on.
- Only omit it when the user explicitly fixes the final trade target in advance and no ranking/selection step is needed.
- For KR intraday opening-strength / momentum / rotation / relative-strength prompts, assume the orchestrator will need `artifacts/symbol_scores.json`.

Recommended pattern inside generated `signal_engine.py`:
...
- [example_signal_engine_kr_orchestrator.py](example_signal_engine_kr_orchestrator.py) — KR multi-symbol opening-strength ranking with `export_latest_signal_scores(...)`
...
- [ ] If the strategy ranks symbols for an orchestrator flow, it also writes `artifacts/symbol_scores.json`
- [ ] If this is a KR multi-symbol orchestrator request, `export_latest_signal_scores(...)` is present unless the final symbol is already fixed
...
- [example_signal_engine_kr_orchestrator.py](example_signal_engine_kr_orchestrator.py) — KR orchestrator-ready sample
