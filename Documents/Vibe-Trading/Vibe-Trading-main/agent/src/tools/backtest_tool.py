...
def run_backtest(run_dir: str) -> str:
    ...
    exit_policy_error = validate_exit_policy(config)
    if exit_policy_error:
        return json.dumps({"status": "error", "error": exit_policy_error}, ensure_ascii=False)
    ...
    if result.success and mode in {"paper", "live"}:
        extra_artifacts = write_paper_mode_artifacts(run_path, config)
        artifacts_found.update({name: str(path) for name, path in extra_artifacts.items()})
    return json.dumps({
