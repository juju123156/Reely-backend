"""Tests for GlobalEquityEngine (US / HK / KR) market rules.
...
def _kr_engine(**overrides) -> GlobalEquityEngine:
    config = {"initial_cash": 500_000}
    config.update(overrides)
    return GlobalEquityEngine(config, market="kr")
...
class TestCanExecute:
    ...
    def test_kr_long(self) -> None:
        assert _kr_engine().can_execute("005930.KS", 1, _make_bar()) is True
...
class TestRoundSize:
    ...
    def test_kr_matches_current_placeholder_rounding(self) -> None:
        engine = _kr_engine()
        assert engine.round_size(10.567, 70_000.0) == 10.57
...
class TestCommission:
    ...
    def test_kr_current_placeholder_zero_commission(self) -> None:
        engine = _kr_engine()
        assert engine.calc_commission(10.0, 70_000.0, 1, is_open=True) == 0.0
...
class TestMarketParam:
    ...
    def test_kr_market(self) -> None:
        engine = GlobalEquityEngine({"initial_cash": 100_000}, market="kr")
