...
Market rules:
  ...
  KR:
    - Phase-1 placeholder routing only
    - Uses US-like execution defaults until KR-specific rules are added
"""
...
class GlobalEquityEngine(BaseEngine):
    ...
    def can_execute(self, symbol: str, direction: int, bar: pd.Series) -> bool:
        """US/HK/KR: T+0 placeholder, both directions allowed."""
        return True

    def round_size(self, raw_size: float, price: float) -> float:
        """US/KR: fractional shares (0.01). HK: 100-share lots."""
        if self.market == "hk":
            return max(int(raw_size / 100) * 100, 0)
        return round(max(raw_size, 0.0), 2)

    def calc_commission(self, size: float, price: float, direction: int, is_open: bool) -> float:
        """US/KR: zero commission. HK: stamp tax + levies."""
        if self.market == "hk":
            notional = size * price
            ...
            return comm
        # KR currently shares the same placeholder defaults as US.
        return 0.0

    def apply_slippage(self, price: float, direction: int) -> float:
        """US/KR: low slippage. HK: moderate slippage."""
