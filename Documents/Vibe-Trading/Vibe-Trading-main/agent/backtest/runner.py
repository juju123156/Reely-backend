...
def _detect_market(code: str) -> str:
    ...
    Returns:
        Market type (a_share/us_equity/hk_equity/kr_equity/crypto/futures/forex);
...
def main(run_dir: Path) -> None:
    ...
    os.environ["VIBE_TRADING_RUN_DIR"] = str(run_dir.resolve())
    os.environ["VIBE_TRADING_CONFIG_PATH"] = str(config_path.resolve())
...
def _create_market_engine(source: str, config: dict, codes: List[str]):
    ...
    if source in ("okx", "ccxt"):
        ...
        if markets & {"us_equity", "hk_equity", "kr_equity"}:
...
def _detect_submarket(codes: List[str]) -> str:
    ...
    Returns:
        "hk" if any code ends with .HK, "kr" if any code ends with .KS/.KQ,
        else "us".
    """
    for code in codes:
        if code.upper().endswith(".HK"):
            return "hk"
        if code.upper().endswith((".KS", ".KQ")):
            return "kr"
    return "us"
