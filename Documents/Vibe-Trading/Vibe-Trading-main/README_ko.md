...
현재 프로젝트는 한국장에서 **리서치 / 전략 생성 / 백테스트** 용도로 사용할 수 있습니다.  
실주문 자동매매는 아직 브로커 API(예: KIS, 키움) 연동이 없으므로 별도 구현이 필요합니다.

### 지원 범위

- KOSPI / KOSDAQ 종목 감지
- KOSPI / KOSDAQ 호가단위 분리
- ETF / ETN 별도 상품 타입 처리
- 현물 결제 `T+2`
- **롱 포지션 당일 재매도 허용**
- **공매도 기본 비활성화**, covered short만 옵션으로 허용
- 매도 시 거래세 반영
  - KOSPI 일반주: 기본 `0.05%`
  - KOSDAQ 일반주: 기본 `0.20%`
  - ETF / ETN: 기본 `0%`
...
한국 종목은 반드시 거래소 suffix를 붙여 사용하는 것을 권장합니다.

- KOSPI: `005930.KS`, `035420.KS`
- KOSDAQ: `091990.KQ`
- ETF: `069500.KS`
- ETN: `500001.KS` 같은 형식

> `.KS` / `.KQ`가 없으면 한국장이 아닌 다른 시장으로 오인되거나, 기본값으로 잘못 라우팅될 수 있습니다.

### 가장 쉬운 사용 방법

채팅 또는 CLI에서 한국장임을 명시하고 종목 코드를 함께 입력합니다.

```text
005930.KS를 한국장 기준으로 2024년 이동평균 돌파 전략 백테스트해줘
```

```text
069500.KS ETF를 한국장 규칙으로 테스트하고 결과를 비교해줘
```
...
한국장 엔진은 아래 순서로 상품 타입을 판별합니다.

1. `krx_instrument_types`에 사용자가 명시한 값
2. 로컬 캐시 `~/.vibe-trading/cache/krx_symbol_master.json`
3. `pykrx` 조회
4. 실패 시 일반주(`stock`)로 처리

ETF / ETN을 확실히 고정하고 싶다면 `config.json`에 직접 지정하세요.

```json
{
  "krx_instrument_types": {
    "069500.KS": "etf",
    "500001.KS": "etn"
  }
}
...
covered short를 시험하려면 명시적으로 켜야 합니다.

```json
{
  "allow_covered_short": true,
  "covered_short_symbols": ["005930"],
  "allow_short_etp": false
}
```

- `allow_covered_short`: covered short 허용 여부
- `covered_short_symbols`: short를 허용할 종목 코드 목록(숫자만)
- `allow_short_etp`: ETF / ETN short 허용 여부
...
- 실주문 자동매매 브로커 연동은 아직 없음
- ETF / ETN 분류는 캐시 / 조회 실패 시 수동 지정이 가장 안전
- 브로커별 위탁수수료는 다르므로 `commission_rate`는 사용자 환경에 맞게 조정 필요

### 운영 팁

- 일반주는 `005930.KS`처럼 suffix를 항상 포함
- ETF / ETN은 `krx_instrument_types`를 명시하는 것이 가장 안전
- 한국장 전략은 먼저 백테스트로 검증한 후 브로커 API 연동을 고려
